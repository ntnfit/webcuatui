...
:root {
    --bg-color: #ffffff;
    --text-color: #1f2937;
    --link-color: #2563eb; /* Màu link light mode */
    --link-hover-color: #1d4ed8; /* Màu link hover light mode */
...
.dark {
    --bg-color: #1f2937;
    --text-color: #e5e7eb;
    --link-color: #3b82f6; /* Màu link dark mode */
    --link-hover-color: #2563eb; /* Màu link hover dark mode */
...
table {
    ...
    margin: 1.5rem 0;
    background-color: var(--bg-color);
    border: 1px solid #6b7280; /* viền bảng chính */
    border-radius: 0.5rem;
    overflow: hidden;
    box-shadow: 0 0 0 1px #6b7280; /* thêm viền bao */
}
...
/* Style cho danh sách không thứ tự (ul) và có thứ tự (ol) */
ul, ol {
...
.dark ol,
.dark li {
    color: var(--text-color); /* tự động là màu text dark mode nhờ biến */
}
...
const BlogDetail: React.FC = () => {
    const { blog, latestBlogs, relatedBlogs, navigation, error } = usePage()
    ...
    useEffect(() => {
        if (blog?.body) {
            ...
            // Thêm class cho các thẻ pre
            content = content.replace(
                /<pre([^>]*)>/g,
                '<pre$1 class="relative">',
            );

            // Thêm class cho các thẻ code
            content = content.replace(
                /<code([^>]*)>/g,
                '<code$1 class="hljs">',
            );
    ...
    useEffect(() => {
        if (blogContentRef.current) {
            // Thêm một chút delay để đảm bảo DOM đã được render đầy đủ
            const highlightTimer = setTimeout(() => {
                const codeBlocks =
                    blogContentRef.current?.querySelectorAll("pre code");

                if (!codeBlocks || codeBlocks.length === 0) return;

                codeBlocks.forEach((codeBlock, index) => {
                    // Kiểm tra xem code block đã được xử lý chưa
                    if (
                        (codeBlock.parentElement as HTMLElement).getAttribute(
                            "data-highlighted",
                        ) === "true"
                    ) {
                    ...
                    if (parent) {
                        ...
                        // Lưu lại các attributes quan trọng của parent (như className)
                        const parentClasses = parent.className;

                        parent.innerHTML = "";
                        parent.className = parentClasses; // Khôi phục class

                        // Thêm lại các class quan trọng nếu bị mất
                        if (!parent.className.includes("dark:bg-gray-800")) {
                            parent.className +=
                                " dark:bg-gray-800 dark:text-gray-200 relative";
                        }
            ...
            }, 300); // Thêm một delay nhỏ để đảm bảo DOM đã sẵn sàng
                                ...
                                <div className="rounded-xl overflow-hidden shadow-lg border border-gray-100 dark:border-gray-800 transition-all duration-300 group-hover:shadow-xl">
                                    ...
                                    {blog.type && (
                                        <div className="absolute top-4 left-4">
                                            <span
                                                className={`inline-block px-3 py-1 rounded-full text-sm font-medium ${getTypeColor(displayType).light} ${getTypeColor(displayType).dark}`}
                                            >
                                                {displayType}
                                            </span>
                                        </div>
                                                            ...
                                                            <img
                                                                ...
                                                                onError={(
                                                                    ...
                                                                    (
                                                                        ...
                                                                        "https://via.placeholder.com/100?text=Blog";
                                                                }}
                                                            />
                                                        </div>
                                                    )}

...
import { Search, Star, ChevronLeft, ChevronRight, FileText, Megaphone, Lightbulb, ArrowRight } from 'lucide-react';
...
const createSlug = (text: string): string => {
    return text
        ...
        .replace(/đ/g, 'd').replace(/Đ/g, 'D') // Xử lý đặc biệt cho chữ đ/Đ
...
const Blogs: React.FC = () => {
    const { posts, pagination, filters, categories } = usePage().props as unknown as PageProps;
    const [searchQuery, setSearchQuery] = useState(filters.search || '');
    const [selectedType, setSelectedType] = useState(filters.type ? typeMapping[filters.type] || '' : '');
    ...
    const allCategories = categories.includes('Tất cả') ? categories : ['Tất cả', ...categories];
    ...
    const handleCategorySelect = (category: string) => {
        ...
        console.log('Category slugs:', newSelectedCategories.map(cat => createSlug(cat)));
        console.log('Params being sent:', params);
    ...
    const paginate = (pageNumber: number) => {
        ...
        if (selectedType) params.type = typeReverseMapping[selectedType] || selectedType;
            ...
            <div className="pt-16">
                <div className="max-w-7xl mx-auto px-4 py-8">
                    <motion.div
                        ...
                        className="mb-8"
                    >
                        <h1 className="text-4xl font-bold text-apple-dark-gray dark:text-white mb-8 transition-colors duration-500">Blog</h1>

                        {/* Type Filters */}
                        <div className="flex flex-wrap md:flex-nowrap gap-2 mb-6 overflow-x-auto pb-2">
                            <motion.div
                                initial="notSelected"
                                animate={selectedType === '' ? "selected" : "notSelected"}
                                variants={filterVariants}
                                className="px-1 py-1"
                            >
                                <Button
                                    variant={selectedType === '' ? "default" : "outline"}
                                    className={`rounded-full border px-4 py-2 ${selectedType === ''
                        ...
                        {/* Categories */}
                        <div className="mb-8">
                            <h2 className="text-xl font-semibold mb-4 text-apple-dark-gray dark:text-white transition-colors duration-500">Danh mục</h2>
                            <div className="flex flex-wrap gap-2">
                                {/* Nút "Tất cả" */}
                                <motion.div
                                    key="all"
                                    initial="notSelected"
                                    animate={selectedCategories.length === 0 ? "selected" : "notSelected"}
                                    variants={filterVariants}
                                ...
                                >
                                    <Button
                                        ...
                                        className={`rounded-full border px-4 py-2 ${selectedCategories.length === 0
                                            ? 'bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-800/50 dark:hover:bg-blue-900/40'
                                            : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-600'
                                            } transition-colors duration-300`}
                                ...
                                {allCategories.filter(category => category !== 'Tất cả').map(category => {
                                    const colors = getCategoryColor(category);
                                    return (
                                        <motion.div
                                            key={category}
                                            initial="notSelected"
                                            animate={selectedCategories.includes(category) ? "selected" : "notSelected"}
                                            variants={filterVariants}
                                            whileHover={{ scale: 1.05 }}
                                            className="px-1 py-1"
                                        >
                        ...
                        <div className="text-gray-600 dark:text-gray-400 mb-6 transition-colors duration-500">
                            {posts.length > 0 ? (
                                <>Hiển thị {(pagination.currentPage - 1) * pagination.perPage + 1} đến {Math.min(pagination.currentPage * pagination.perPage, pagination.total)} trong tổng số {pagination.total} kết quả</>
                            ) : (
                            ...
                            {posts.map((post, index) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        animate="visible"
                                        custom={index}
                                        className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden hover:shadow-md dark:hover:shadow-gray-800/50 transition-all duration-300"
                                        whileHover={{ y: -5, transition: { duration: 0.3 } }}
                                    >
                                        {/* Hình ảnh với thứ tự ưu tiên: thumbnail > featured_image > placeholder */}
                                        <Link href={`/blogs/${post.slug}`} className="block overflow-hidden h-48 relative">
                                            <img
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <h3 className="mb-2">
                                                        <Link
                                                            ...
                                                            className="text-xl font-semibold text-apple-dark-gray dark:text-white line-clamp-2 transition-colors duration-500 hover:text-apple-blue dark:hover:text-blue-400"
                                                        >
                                                            {post.title}
                                                        </Link>
                                                    </h3>
                        ...
                        <div className="flex items-center justify-center gap-2 mt-8">
                            <Button
                                variant="outline"
                                size="icon"
                                onClick={() => pagination.currentPage > 1 && paginate(pagination.currentPage - 1)}
                            ...
                            <Button
                                variant="outline"
                                size="icon"
                                onClick={() => pagination.currentPage < pagination.lastPage && paginate(pagination.currentPage + 1)}
